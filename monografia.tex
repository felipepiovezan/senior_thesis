...
%Dá uma olhada na matplotlib   /numpy
	...
	\li \If $x_1 \not= x_2$ 
	\Do	
		\li Set $\lambda \gets (y_1 - y_2)/(x_1 - x_2)$
	\li \Else
		\li \If $y_1 \not= y_2$ or $y_2 = 0$
		\Do
			\li Output $O$ and stop.
		\End
		\li Set $\lambda \gets (3{x_2}^2 + a)/(2y_2)$
	\End
	\li Set $x_3 \gets \lambda^2 - x_1 - x_2 \quad mod\; p$
	\li Set $y_3 \gets (x_2 - x_3)\lambda - y_1 \quad mod\; p$
	\li output $(x_3, y_3)$
    \end{codebox}


	Uma operação de soma pode ser definida sobre uma curva elíptica de modo a torná-la um grupo e, assim, poderemos definir o problema do logaritmo discreto sobre esse conjunto. Por clareza, definimos a soma de dois elementos de $E$ para o caso de $GF(p)$ através do algoritmo \proc{Soma}, conforme \cite{IEE1363}.
