...
Pode-se, então, definir uma função responsável por mapear uma mensagem e uma chave para uma nova string, a qual espera-se que seja ``aleatória'' (uma definição precisa para esse termo será dada ). De modo similar, precisa-se de uma função que desfaça o trabalho da primeira, isto é, uma função que receba a string aleatória junto com a chave utilizada para gerá-la e produza a mensagem original. Tais idéias são capturas pelas seguintes definições:
